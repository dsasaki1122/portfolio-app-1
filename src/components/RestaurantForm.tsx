...
type ImageSlot = {
  ...
  key?: string;
  error?: string;
  // "existing": 編集画面を開いた時点で既に保存されていた画像（保存するまではS3から削除しない）。
  // "new": このセッション中にアップロードした画像（削除したら即S3からも消す）。
  origin: "existing" | "new";
};
...
export default function RestaurantForm({ mode, restaurantId, initialValues }: Props) {
  ...
  // ローカルプレビュー用の object URL をアンマウント時に解放（既存画像は blob: ではないため対象外）
  useEffect(() => {
    return () => {
      images.forEach((img) => {
        if (img.origin === "new") URL.revokeObjectURL(img.previewUrl);
      });
    };
  }, [images]);
  ...
  const handleRemoveImage = (id: string) => {
    ...
    if (target.origin === "existing") {
      // 保存を押すまではS3側は消さない（保存時にサーバー側で差分削除される）
      return;
    }
  ...
  const handleSubmit: React.FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    const requestUrl =
      mode === "edit" ? `/api/restaurants/${restaurantId}` : "/api/restaurants";
    const method = mode === "edit" ? "PATCH" : "POST";
    const fallbackErrorMessage =
      mode === "edit" ? "更新に失敗しました" : "登録に失敗しました";
  ...
  const submitLabel = mode === "edit" ? "更新" : "登録";
  const submittingLabel = mode === "edit" ? "更新中..." : "登録中...";
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  name="station"
                  type="text"
                  defaultValue={initialValues?.nearestStation}
                  className="w-full px-3 py-2 border rounded"
                />
              </div>
              ...
              <div>
                ...
                <input
                  name="visitDate"
                  type="date"
                  defaultValue={initialValues?.visitedAt}
                  className="w-full px-3 py-2 border rounded"
                />
              </div>
